...
<div class="md:flex">
    ...
    <main class="w-full px-5 md:px-20 bg-gray-100 ">
        <h2 class="text-xl py-10">Dashbord / Athletes</h2>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            ...
                            if (!in_array($uploadedExtension, $allowedExtensions)) {
                                ...
                                if (move_uploaded_file($photoTmpName, $targetPath)) {
                                    ...
                                    $result = $add->save("athletes", $userData);
                                    
                                    if ($result['status'] == 'success') {
                                        $response = $result['message'];
                                    } else {
                                        $response = "Failed to add Athlete.";
                                    }
                                } else {
                            ...
                            echo "<script>alert('$message'); window.history.pushState({}, '', 'athletes'); window.location.reload();</script>";
                        }
                        ?>
                        ...
                        <div class="container px-0 mx-auto sm:px-5 bg-white p-5 md:w-1/5 rounded-lg shadow-lg md:mt-20">
                            <div class="md:w-full pb-5">
                                ...
                                <div class="py-5 flex justify-center items-cenetr">
                                    <h2 class="text-2xl font-bold text-gray-600">Add Athlete Here</h2>
                                </div>
                                <form action="" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class=" mb-4 px-3">
                                        <input type="text" id="name" required name="email" placeholder="Enter Email address" class="w-full  py-1.5 px-6 bg-white outline-none border border-gray-300 rounded ">
                                    </div>
                                    <div class=" mb-4 px-3">
                                    ...
                                    <div class=" mb-4 px-3">
                                        <input type="text" id="name" required name="club" placeholder="Enter Club" class="w-full  py-1.5 px-6 bg-white outline-none border border-gray-300 rounded ">
                                    </div>
                                    <div class=" mb-4 px-3">
                                        <input type="file" id="name"  name="photo" placeholder="Choose Photo" class="w-full  py-1.5 px-6 bg-white outline-none border border-gray-300 rounded ">
                                    </div>
			...
			<table id="datatable" class=" table datatable stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
				<thead class="text-left px-5">
					<tr class="bg-gray-100">
						...
						<th class="py-2 px-5 border" data-priority="1">Email</th>
						<th class="py-2 px-5 border" data-priority="1">Phone</th>
						<th class="py-2 px-5 border" data-priority="2">Address</th>
						<th class="py-2 px-5 border" data-priority="2">Club</th>
				...
				<?php
                        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
							...
							<tr>
								...
								<td class="px-5 py-1 border-b"><?php echo $user['club']; ?></td>
